...
export function ValueGrid() {
  return (
    <section aria-label="Our values" className="py-24 bg-brand-black relative border-t border-brand-silver/5 overflow-hidden">
      <SectionSettle className="container mx-auto px-6">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-16 max-w-6xl mx-auto">
          <div className="max-w-xl">
            <BlurReveal delay={0}>
              <p className="text-brand-blue font-semibold tracking-[0.2em] text-[10px] md:text-xs uppercase mb-5 flex items-center gap-4">
                <span className="w-8 h-[1px] bg-brand-blue" />
                Engineering Excellence
              </p>
            </BlurReveal>
            <BlurReveal delay={0.1}>
              <h2 className="font-display text-3xl md:text-4xl lg:text-5xl font-medium tracking-tight text-brand-white text-balance">
                Unequivocally superior <br className="hidden md:block" />
                <span className="text-brand-silver/60">craftsmanship.</span>
              </h2>
            </BlurReveal>
          </div>

          <BlurReveal delay={0.2}>
            <a href="#expertise" className="flex items-center gap-2 text-brand-white/70 font-medium uppercase tracking-wider text-xs hover:text-brand-white transition-colors duration-300 group">
              View Specifications
        ...
        <div className="grid md:grid-cols-3 gap-5 max-w-6xl mx-auto">
          {values.map((val, i) => (
            <motion.div
              ...
              className="group relative overflow-hidden rounded-xl glass-panel h-[420px] md:h-[460px] p-7 flex flex-col justify-end cursor-pointer"
            >
              ...
              <div className="absolute top-7 left-7 right-7 flex justify-between items-start z-20">
                <span className="text-brand-silver/20 font-display text-lg">0{i + 1}</span>
                <ArrowUpRight className="w-5 h-5 text-brand-silver/30 group-hover:text-brand-yellow transform group-hover:-translate-y-0.5 group-hover:translate-x-0.5 transition-all duration-500" />
              </div>

              <div className="relative z-20 w-full mt-auto">
                <h3 className="font-display text-xl font-medium tracking-tight text-brand-white mb-3">{val.title}</h3>
