...
export function TrustTicker() {
  return (
    <div className="py-10 bg-bg-base border-y border-surface-muted/10 overflow-hidden flex relative transition-colors duration-300">
      <div className="absolute left-0 top-0 bottom-0 w-40 bg-gradient-to-r from-surface to-transparent z-10" />
      <div className="absolute right-0 top-0 bottom-0 w-40 bg-gradient-to-l from-surface to-transparent z-10" />

      <div className="animate-marquee flex gap-24 items-center px-8 group">
        {[...logos, ...logos, ...logos].map((logo, i) => (
          <div key={i} className="flex items-center gap-3 text-text-secondary opacity-50 group-hover:opacity-20 hover:!opacity-100 hover:!text-text-primary transition-all duration-300 cursor-default">
